...
//        jacad("FRANCE","french");
//        jacad("handshake","shake hands");
//        jacad("aa1+aa2","AAAA12");
        jacad("E=M*C^2","e=m*c^2");
    }

    public static int jacad(String str1, String str2) {
        if(str1.length() < 2 || str1.length() > 1000 || str2.length() < 2 || str2.length() > 1000) throw new RuntimeException("String Length error");
        List<String> str1Array = getList(str1);
        List<String> str2Array = getList(str2);
        List<String> cap = new LinkedList<>();
        ...
        for (String s2 : str2Array) union.add(s2);

        // 둘다 공집합일 경우
        if(cap.size() == 0 && union.size() == 0) {
            cap.add("");
            union.add("");
        }
        System.out.println(Float.valueOf((float)cap.size()/union.size()*65536).intValue());
        return Float.valueOf((float)cap.size()/union.size()*65536).intValue();
    }

    public static List<String> getList(String str) {
        System.out.println(str);
        List<String> strArray = new LinkedList<>();
        Matcher m = Pattern.compile(".{2}").matcher(str);
        int i = 0;
        while (m.find(i)) {
            if (Pattern.matches("[a-zA-Z]{2}", m.group())) strArray.add(m.group());
            i++;
        }
        return strArray;
    }
