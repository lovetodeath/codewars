...
public class SecretMap {
    public static void main(String[] args) throws KakaoException {
        ...
        for(String rtn :rtnMsg) { System.out.println(rtn); }
    }

    public static String[] calculator(int n, int[] arr1, int[] arr2) {
        String[] rtnStr = new String[n];
        for(int i=0; i < n; i++) {
            rtnStr[i] = Integer.toBinaryString( arr1[i] | arr2[i]);
            for(int j=0; j < n-rtnStr[i].length(); j++) { rtnStr[i] = "0" + rtnStr[i]; }
            rtnStr[i] = rtnStr[i].replace("0", " ").replace("1", "#");
        }
        return rtnStr;
    }
}